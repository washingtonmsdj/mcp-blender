...
for row, z in enumerate(row_centers, start=1):
    for col, v in enumerate(bay_centers, start=1):
        ...
        box(f"caixilho horizontal {row}-{col}", U_FACE + 0.072, v, z, 0.025, window_width - 0.10, 0.055, ivory)
for col, v in enumerate(bay_centers, start=1):
    box(f"painel decorativo fundo {col}", U_FACE + 0.020, v, 47.18, 0.065, 2.02, 1.26, ivory, 0.035)
    box(f"painel decorativo rebaixo {col}", U_FACE + 0.064, v, 47.18, 0.030, 1.58, 0.84, shadow, 0.02)
    box(f"painel decorativo miolo {col}", U_FACE + 0.086, v, 47.18, 0.018, 1.38, 0.66, ivory, 0.015)
for idx, v in enumerate((-4.72, -1.60, 1.60, 4.72), start=1):
    box(f"pilastra frontal {idx}", U_FACE + 0.085, v, 48.0, 0.05, 0.22, 7.65, ivory, 0.01)
box("placa LACERDA", U_FACE + 0.075, 0.0, 51.25, 0.05, 4.25, 0.72, ivory, 0.02)
bpy.ops.mesh.primitive_cylinder_add(vertices=48, radius=0.42, depth=0.07, location=world(U_FACE + 0.12, 0.0, 51.92))
